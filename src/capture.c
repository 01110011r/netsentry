...
int capture_init(const char *iface, char *errbuf_out) {
    ...
    if (!g_pcap) {
        if (errbuf_out) snprintf(errbuf_out, PCAP_ERRBUF_SIZE, "%s", errbuf);
        return -1;
    }
...
static int parse_packet(const struct pcap_pkthdr *hdr, const u_char *bytes,
                         packet_info_t *out) {
    if (hdr->caplen < sizeof(struct ether_header)) return -1;

    const struct ether_header *eth = (const struct ether_header *)bytes;
    if (ntohs(eth->ether_type) != ETHERTYPE_IP) return -1; /* skip ARP, IPv6, etc. for now */

    size_t offset = sizeof(struct ether_header);
    if (hdr->caplen < offset + sizeof(struct ip)) return -1;

    const struct ip *iph = (const struct ip *)(bytes + offset);
    size_t ip_hlen = iph->ip_hl * 4u;
    if (ip_hlen < sizeof(struct ip) || hdr->caplen < offset + ip_hlen) return -1;
    ...
    switch (iph->ip_p) {
        case IPPROTO_TCP: {
            if (hdr->caplen < l4_offset + sizeof(struct tcphdr)) break;
        ...
        case IPPROTO_UDP: {
            if (hdr->caplen < l4_offset + sizeof(struct udphdr)) break;
...
static void pcap_callback(u_char *user, const struct pcap_pkthdr *hdr,
    ...
    if (parse_packet(hdr, bytes, &pkt) != 0) return; /* not IPv4, skip */
    ctx->handler(ctx->user_ctx, &pkt);
}

int capture_run(packet_handler_fn on_packet, void *user_ctx) {
    if (!g_pcap) {
